...
{	
	public static void Main()
	{
		...
		while (i < polymer.Count() - 1)
		{
			...
			if (!pairs.ContainsKey(adjacent))
			{		
				pairs[adjacent] = 0;
			}
			
			pairs[adjacent] = pairs[adjacent] + 1;
			i++;
		}
		...
		Dictionary<char, double> charCounts = new Dictionary<char, double> ();
		foreach(var pair in pairs)
		{
			if (!charCounts.ContainsKey(pair.Key[0]))
			{
				charCounts[pair.Key[0]] = 0;
			}
			charCounts[pair.Key[0]] = charCounts[pair.Key[0]] + pair.Value;
		}
		
		charCounts[polymer.Last()] =  charCounts[polymer.Last()] +1;		
		Console.WriteLine(charCounts.OrderBy(c => c.Value).Last().Value - charCounts.OrderBy(c => c.Value).First().Value);
	}
	
	public static Dictionary<string, double> applyRules(Dictionary<string, double> pairs, Dictionary<string, char> rules)
	{
		...
		foreach(var pair in pairs)
		{
			...
			{	
				string newPairKey = new string(new char[] {pair.Key[0], rules[pair.Key]});
				if (!newPairs.ContainsKey(newPairKey))
				{
					newPairs[newPairKey] = 0;
				}
				
				newPairs[newPairKey] = newPairs[newPairKey] + pair.Value;
				
				newPairKey = new string(new char[] {rules[pair.Key], pair.Key[1]});
				if (!newPairs.ContainsKey(newPairKey))
				{
					newPairs[newPairKey] = 0;
				}
